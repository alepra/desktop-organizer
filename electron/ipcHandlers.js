...
ipcMain.handle("get-desktop-icon-positions", async () => {
  try {
    // Find native helper EXE (try Release first, then Debug)
    const repoRoot = path.join(__dirname, "..");
    const releasePath = path.join(repoRoot, "native-helper", "bin", "Release", "net8.0", "IconPositionHelper.exe");
    const debugPath = path.join(repoRoot, "native-helper", "bin", "Debug", "net8.0", "IconPositionHelper.exe");
    ...
    if (fs.existsSync(releasePath)) {
      ...
      console.error("[NATIVE] Native helper EXE not found at Release or Debug path");
      console.error(`[NATIVE] Tried: ${releasePath}`);
      console.error(`[NATIVE] Tried: ${debugPath}`);
      return [];
    }

    // Run native helper EXE and capture STDOUT
